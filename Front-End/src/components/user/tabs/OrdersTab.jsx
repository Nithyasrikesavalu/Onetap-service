...
export default function OrdersTab() {
        ...
        const mappedOrders = data.orders.map((order) => ({
          ...
          documents:
            order.documents && order.documents.length
              ? order.documents.map((doc) => doc.originalName || doc)
              : [],
          vendorNotes:
            order.vendorNotes || order.notes || "No vendor updates yet.",
  ...
  return (
    <div className="space-y-6 p-8 min-h-screen">
      ...
      {/* Orders Table */}
      <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
        <div className="grid grid-cols-11 gap-6 px-6 py-4 bg-gray-50 border-b border-gray-200 text-sm font-medium text-gray-700">
          <div className="col-span-3">Order Details</div>
          <div className="col-span-2">Vendor</div>
        ...
        <div className="divide-y divide-gray-200">
          {sortedOrders.map((order) => {
            ...
            return (
              <div
                key={order.id}
                className="grid grid-cols-11 gap-4 px-6 py-4 hover:bg-gray-50 transition-colors cursor-pointer"
                onClick={() => setSelectedOrder(order)}
              >
                {/* Order Info */}
                <div className="col-span-3">
