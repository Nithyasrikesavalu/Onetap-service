...
const mockVendorAvailability = {
  ...
  bookedSlots: {
    [new Date().toISOString().split("T")[0]]: ["10:00", "14:30"],
...
function FloatingActionButton({ onClick, icon, label, variant = "primary" }) {
  const baseClasses =
    "fixed z-40 w-14 h-14 rounded-full shadow-2xl flex items-center justify-center text-white hover:scale-110 transition-transform duration-300 group";

  const variants = {
    primary: "bg-gradient-to-r from-blue-600 to-indigo-700",
    secondary: "bg-gradient-to-r from-green-600 to-emerald-700",
  };

  const positions = {
    primary: "bottom-6 right-6",
    secondary: "bottom-24 right-6",
  };

  return (
    <button
      onClick={onClick}
      className={`${baseClasses} ${variants[variant]} ${positions[variant]}`}
      aria-label={label}
    >
...
function ServiceManagementModal({ isOpen, onClose, editingService = null }) {
  ...
  const allAvailableFeatures = [
    ...new Set(
      Object.values(documentRequirements)
        .flat()
        .concat(Object.keys(documentRequirements))
    ),
  ];
  ...
  useEffect(() => {
    if (editingService) {
      ...
      setSelectedFeatures(editingService.extras || []);
    } else {
      setFormData({
        title: "",
        sub: "",
  ...
  const handleAddExtra = () => {
    if (newExtra.trim() && !formData.extras.includes(newExtra.trim())) {
      setFormData((prev) => ({
        ...prev,
  ...
  const handleSubmit = (e) => {
    ...
    const finalExtras = [...new Set([...formData.extras, ...selectedFeatures])];
    const serviceData = {
      ...formData,
      extras: finalExtras,
    };
  ...
  const renderBasicInfoStep = () => (
    <div className="space-y-6">
      <div className="text-center mb-6">
        <div className="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-4">
          <Icon name="doc" className="w-8 h-8 text-blue-600" />
        </div>
        <h3 className="text-xl font-semibold text-gray-900 mb-2">
          {editingService ? "Edit Service" : "Add New Service"}
        </h3>
        <p className="text-gray-600">
          Enter basic information about your service
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors text-sm"
            placeholder="e.g., ஆவண அச்சிடுதல்"
            required
          />
        </div>
      </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {availableIcons.map((icon) => (
              ...
              >
                <Icon name={icon} className="w-5 h-5 mx-auto" />
  ...
  const renderFeaturesStep = () => (
    <div className="space-y-6">
      <div className="text-center mb-6">
        <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
          <Icon name="id" className="w-8 h-8 text-green-600" />
        </div>
        <h3 className="text-xl font-semibold text-gray-900 mb-2">
          Service Features
        </h3>
        <p className="text-gray-600">Add features and sub-services</p>
      </div>
      ...
      <div>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="px-6 py-3 bg-blue-500 text-white rounded-xl hover:bg-blue-600 transition-colors font-medium text-sm"
          >
            Add
          </button>
        </div>
      ...
      <div>
        ...
        <div className="max-h-60 overflow-y-auto border border-gray-300 rounded-xl p-4">
          <div className="grid grid-cols-1 gap-2">
            {allAvailableFeatures.map((feature, index) => (
              <label
                key={index}
              ...
              >
                ...
                <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">
                  {physicalAppointmentServices.includes(feature)
                    ? "Physical"
                    : onlineAppointmentServices.includes(feature)
                    ? "Online"
                    : "Both"}
                </span>
              </label>
    ...
    <div className="space-y-6">
      <div className="text-center mb-6">
        <div className="w-16 h-16 bg-purple-100 rounded-full flex items-center justify-center mx-auto mb-4">
          <Icon name="card" className="w-8 h-8 text-purple-600" />
        </div>
        <h3 className="text-xl font-semibold text-gray-900 mb-2">
          Service Preview
        </h3>
        <p className="text-gray-600">Review your service before saving</p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg border overflow-hidden">
        ...
        >
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              <div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center backdrop-blur-sm">
                <Icon name={formData.icon} className="w-6 h-6 text-white" />
              </div>
              <div className="text-white/80 text-sm font-medium">
                {formData.sub}
              </div>
            ...
            <p className="text-white/90 text-sm">{formData.desc}</p>
          </div>
        </div>

        <div className="p-6">
          <div className="space-y-3">
            {[...formData.extras, ...selectedFeatures]
              .slice(0, 3)
              .map((extra, i) => (
                <div
                  key={i}
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      {onlineAppointmentServices.includes(extra) &&
                        !physicalAppointmentServices.includes(extra) && (
                          <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full">
                            Online
                          </span>
            ...
            {[...formData.extras, ...selectedFeatures].length > 3 && (
              <div className="py-3 bg-gray-100 rounded-xl text-center">
                <span className="text-sm text-gray-600 font-medium">
                  + {[...formData.extras, ...selectedFeatures].length - 3} more
                  features
                </span>
              </div>
            )}
      ...
      <div className="bg-gray-50 rounded-xl p-4">
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between">
            <span className="text-gray-600">Tamil Title:</span>
            <span className="font-medium">{formData.sub}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600">Description:</span>
            <span className="font-medium">{formData.desc}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600">Total Features:</span>
            <span className="font-medium">
              {[...formData.extras, ...selectedFeatures].length}
            </span>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto border border-white/20">
        <div className="sticky top-0 bg-white/95 backdrop-blur-sm p-6 border-b border-gray-200/50 rounded-t-3xl">
          <div className="flex items-center justify-between">
            <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-6">
              ...
              {currentStep < 3 ? (
                <button
                  type="button"
                  onClick={() => setCurrentStep(currentStep + 1)}
                  className="flex-1 px-4 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl hover:from-blue-600 hover:to-indigo-700 transition-all font-medium text-sm shadow-lg"
                >
                  Continue
                </button>
              ) : (
                <button
                  type="submit"
                  className="flex-1 px-4 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl hover:from-green-600 hover:to-emerald-700 transition-all font-medium text-sm shadow-lg"
                >
...
export default function ServiceInput() {
  ...
  const openManagementModal = (service = null) => {
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
            <div className="max-w-4xl mx-auto mb-12">
              <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <button
                  ...
                  className="px-6 py-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-2xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-300 font-medium flex items-center space-x-2 whitespace-nowrap"
                >
              ...
              {filteredServices.map((service, index) => (
                <div key={index} className="group relative">
                  <button
                    ...
                    className="absolute top-4 right-4 z-20 w-8 h-8 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center text-gray-600 hover:text-blue-600 hover:bg-white transition-all duration-300 opacity-0 group-hover:opacity-100 shadow-lg"
                    title="Edit Service"
                  >
                    <svg
                      className="w-4 h-4"
            ...
            {filteredServices.length === 0 && (
              <div className="text-center py-16">
                ...
                <button
                  onClick={() => openManagementModal()}
                  className="px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl hover:from-blue-600 hover:to-indigo-700 transition-all font-medium shadow-lg"
                >
                  Create New Service
                </button>
              </div>
