...
const RequestService = () => {
  const [form, setForm] = useState({
    ...
    appointmentDate: "",
    appointmentTime: "",
    appointmentType: "in-person", // in-person or online
    branchLocation: "",
  });
  ...
  const validateStep = (step) => {
    ...
    switch (step) {
      ...
      case 3:
        ...
        break;
      case 4:
        if (!form.appointmentType) newErrors.appointmentType = "Please select appointment type";
        if (!form.appointmentDate) newErrors.appointmentDate = "Please select appointment date";
        else if (new Date(form.appointmentDate) < new Date().setHours(0,0,0,0)) {
          newErrors.appointmentDate = "Please select a future date";
        }
        if (!form.appointmentTime) newErrors.appointmentTime = "Please select appointment time";
  ...
  const steps = [
    ...
    { title: "Category", icon: "📂", field: "serviceCategory" },
    { title: "Service", icon: "🛠️", field: "serviceType" },
    { title: "Appointment", icon: "📅", fields: ["appointmentDate", "appointmentTime"] },
    { title: "Details", icon: "📝", field: "message" },
  ];
  ...
  const appointmentTypes = [
    { value: "in-person", label: "🏢 In-Person Visit", description: "Visit our branch office" },
    { value: "online", label: "💻 Online Consultation", description: "Video call or chat" }
  ];

  const branchLocations = [
    { value: "t Nager", label: "t Nager", address: "123 MG Road, t Nager" },
    { value: "C", label: "Tambaram ", address: "456 Brigade Road, Tambaram" },
    { value: "Tambaram", label: "Tambaram", address: "789 Marine Drive, Tambaram" },
    { value: "Porur", label: "Porur", address: "321 Connaught Place,Porur" },
    { value: "Guindy", label: "Guindy", address: "654 Hi-Tech City, Guindy" }
  ];
  ...
  // Get next 30 days for appointment dates
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-900 via-indigo-900 to-purple-900 text-white px-4 sm:px-6 relative overflow-hidden">
      ...
      >
        ...
        <div className="flex justify-between items-center mb-8 relative">
              ...
              >
                ...
                {isStepComplete(index) && (
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    className="absolute -top-1 -right-1 w-3 h-3 sm:w-4 sm:h-4 bg-cyan-400 rounded-full flex items-center justify-center"
                  >
          ...
          {/* Enhanced Progress Line */}
          <div className="absolute top-4 sm:top-5 left-0 right-0 h-1 bg-white/20 -z-10 rounded-full">
            <motion.div
              className="h-full bg-gradient-to-r from-blue-500 to-cyan-500 rounded-full shadow-lg shadow-blue-500/30"
              initial={{ width: "0%" }}
              animate={{ width: `${(currentStep / (steps.length - 1)) * 100}%` }}
                    ...
                    >
                            ...
                            {branchLocations.map((branch) => (
                              ...
                              >
                                <input
                                  ...
                                  name="branchLocation"
                                  value={branch.value}
                                  checked={form.branchLocation === branch.value}
                                  onChange={handleChange}
                                  className="hidden"
                                />
                                <div className="flex items-center space-x-3 w-full">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <select
                            ...
                            onChange={handleChange}
                            className={`w-full px-4 py-3 rounded-xl border transition-all duration-300 text-white focus:outline-none focus:ring-2 ${
                      ...
                      {form.appointmentDate && form.appointmentTime && (
                        <motion.div
                          initial={{ opacity: 0, y: 20 }}
                          animate={{ opacity: 1, y: 0 }}
                          className="p-4 bg-blue-500/20 border border-blue-400/30 rounded-xl"
                        >
                          <div className="text-center">
                            ...
                            <div className="text-white/80 text-sm mt-1">
                              {formatAppointmentDate(form.appointmentDate)} at {form.appointmentTime}
                            </div>
                            {form.appointmentType === "in-person" && form.branchLocation && (
            ...
            >
              ...
              <div>
                ...
                >
                  Thank you <span className="text-blue-300 font-semibold">{form.name}</span>! 
                  <br />
                  Your appointment for <span className="text-cyan-300 font-semibold">{getSelectedServiceLabel()}</span> has been scheduled.
                  <br /><br />
                  ...
                  <br /><br />
                  Confirmation has been sent to <span className="text-indigo-300">{form.email}</span> and <span className="text-indigo-300">{form.mobile}</span>.
                </motion.p>
              </div>
